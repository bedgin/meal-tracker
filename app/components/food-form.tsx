...
export default function FoodForm({ food, existingFoods, returnTo }: Props) {
  ...
  const [measureAmount, setMeasureAmount] = useState(
    food?.servingMeasureAmount?.toString() ?? ""
  );
  const [measureUnit, setMeasureUnit] = useState(
    food?.servingMeasureUnit ?? "items"
  );
  const [weightAmount, setWeightAmount] = useState(
    food?.servingWeightAmount?.toString() ?? ""
  );
  const [weightUnit, setWeightUnit] = useState(food?.servingWeightUnit ?? "g");
  ...
  function fillFromFood(f: Food) {
    setName(f.name);
    setMeasureAmount(f.servingMeasureAmount?.toString() ?? "");
    setMeasureUnit(f.servingMeasureUnit ?? "items");
    setWeightAmount(f.servingWeightAmount?.toString() ?? "");
    setWeightUnit(f.servingWeightUnit ?? "g");
    setCalories(f.caloriesPerServing.toString());
    setProtein(f.proteinPerServing.toString());
  ...
  return (
    <main className="min-h-screen bg-gray-50 max-w-lg mx-auto flex flex-col">
              ...
              {filtered.map((f) => (
                ...
                >
                  ...
                  <span className="block text-xs text-gray-400 mt-0.5">
                    {f.caloriesPerServing} cal · {f.proteinPerServing}g protein
                  </span>
                </button>
              ))}
            </div>
      ...
      <div className="px-4 pb-10 pt-2 space-y-3">
        ...
        >
          {isPending ? "Saving…" : isEditing ? "Save Changes" : "Save Food"}
        </button>
        ...
        {isEditing && (
          ...
          >
            {deleteConfirm ? "Tap again to confirm delete" : "Delete Food"}
          </button>
        )}
      </div>
    </main>
